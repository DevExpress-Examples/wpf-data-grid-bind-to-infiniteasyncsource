...
Namespace InfiniteAsyncSourceSample
    Public NotInheritable Class IssuesService
        ...
        Public Async Shared Function GetSummariesAsync(ByVal filter As IssueFilter) As Task(Of IssuesSummaries)
            Await Task.Delay(300)
            Dim issues = DirectCast(AllIssues.Value, IEnumerable(Of IssueData))
            If filter IsNot Nothing Then
                issues = FilterIssues(filter, issues)
            End If
            Dim lastCreated = If(issues.Any(), issues.Max(Function(x) x.Created), Nothing)
